...
interface NoteDrawPluginLike {
  ...
  scheduleDrawingSave?: (file?: { path?: string }, data?: unknown) => void;
}
...
export default class MobileWebviewerPlugin extends Plugin {
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        for (const node of Array.from(mutation.addedNodes)) {
          ...
          if (!root) {
            this.cleanupStaleNoteDrawButtonResidue(node.closest<HTMLElement>(".workspace-leaf-content") ?? this.app.workspace.containerEl);
            continue;
          }
    ...
    for (const leaf of leaves) {
      ...
      leaf.querySelectorAll<NoteDrawButtonElement>(".view-actions .notedraw-header-button").forEach((button) => {
        ...
        if (hadMobileWebviewerState) {
          ...
          if (hadHeaderProxy || staleWebviewController) {
  ...
  activateNoteDrawWebviewController(controller: NoteDrawControllerLike, surface: HTMLElement): boolean {
    ...
    if (!previewEl?.isConnected || !this.isMobileWebviewerSurface(previewEl)) return false;
    ...
    const openDirectly = () => {
      ...
      controller.palettePanel?.addClass("is-drawing-active");
      controller.textPanel?.addClass("is-drawing-active");
      controller.selectionMenu?.addClass("is-drawing-active");
      controller.formatToolbar?.addClass("is-drawing-active");
  ...
  forceNoteDrawWebviewToolbarVisible(controller: NoteDrawControllerLike, surface: HTMLElement): void {
    const previewEl = controller.previewEl ?? surface;
    if (!previewEl?.isConnected || !this.isMobileWebviewerSurface(previewEl)) return;
    ...
    for (const element of [controller.toolbar, controller.palettePanel, controller.textPanel, controller.selectionMenu, controller.formatToolbar]) {
      element?.addClass("is-drawing-active");
    }
    ...
    if ((!controller.formatToolbar || !controller.formatToolbar.isConnected) && typeof controller.createFormatToolbar === "function") {
      try {
        ...
        console.warn("[mobile-webviewer] NoteDraw webview format toolbar create skipped", error);
      }
    }
    ...
    this.queueNoteDrawControllerSync(previewEl, true);
  ...
  findNoteDrawSourceButton(root?: HTMLElement): NoteDrawButtonElement | null {
    ...
    for (const scope of scopes) {
      const buttons = Array.from(scope.querySelectorAll<HTMLElement>(NOTEDRAW_BUTTON_SELECTOR)).filter(
        (candidate) => candidate.isConnected && !candidate.hasClass("mwv-notedraw-launcher") && !candidate.hasClass("notedraw-header-button")
      );
  ...
  flushNoteDrawDrawingNow(controller?: NoteDrawControllerLike | null): Promise<void> {
    if (!controller?.file) return Promise.resolve();
    const data = (controller as NoteDrawControllerLike & { drawingData?: unknown }).drawingData;
  ...
  bindNoteDrawWebviewButton(button: NoteDrawButtonElement, controller: NoteDrawControllerLike): void {
    ...
    const handleActivation = (event: Event) => {
      const now = Date.now();
      if (event.type === "click" && button._mwvNoteDrawLastTouchMs && now - button._mwvNoteDrawLastTouchMs < 700) {
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation?.();
        return;
      }
      if (event.type === "touchend") button._mwvNoteDrawLastTouchMs = now;
      const current = button._noteDrawController ?? button._mwvNoteDrawBoundController ?? controller;
      if (current?.surfaceType !== "webview" || !this.isMobileWebviewerSurface(current.previewEl)) return;
      const surface = current.previewEl;
      if (!surface?.isConnected) return;
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation?.();
      this.activateNoteDrawWebviewController(current, surface);
    };
    button.addEventListener("click", handleActivation, { capture: true });
    button.addEventListener("touchend", handleActivation, { capture: true, passive: false });
  }
