...
const checks = [
  ...
  ["NoteWeb wand only adopts NoteDraw's native button and never fabricates a proxy", wandMethod.includes(':scope > .notedraw-webview-button:not([data-mwv-browser-more=\'true\']') && wandMethod.includes("findNoteDrawWebviewSourceButton(surface)") && !wandMethod.includes('anchor.createEl("button"') && !source.includes("decorateNoteDrawWebWandButton")],
