...
const checks = [
  ...
  ["NoteWeb wand proxy resolves its owner outside the embed subtree", source.includes("_mwvNoteWebWandSurface") && source.includes("const current = button?._mwvNoteWebWandSurface") && source.includes("findNoteDrawPreviewController(current, true)")],
  ["NoteWeb raw-page wand keeps Web mode and opens the element editor", wandMethod.includes("this.isNoteBrowserWebMode(current)") && wandMethod.includes("this.toggleNoteWebRawElementEditing(current)") && !wandMethod.includes('setNoteBrowserEmbedMode(current, "note")')],
  ...
  ["NoteWeb Web mode always exposes a host-side magic wand", source.includes("ensureNoteWebWandProxy(surface)") && source.includes("ensureNoteWebWandProxy(embed)") && source.includes("data-mwv-noteweb-wand") && styles.includes(".mwv-notedraw-anchor") && styles.includes(".notedraw-webview-button:not(.mwv-notedraw-web-wand)")],
  ["NoteWeb magic wand stays host-side while its selector explicitly bridges the guest", source.includes("mwv-notedraw-host-proxy") && source.includes("this.noteDrawStableAnchorHost(surface)") && rawEditorMethods.includes("noteWebRawElementEditorScript") && rawEditorMethods.includes("frame.executeJavaScript(code, true)")],
  ...
  ["wand proxy renders only when NoteDraw is installed", wandMethod.includes("if (!this.getNoteDrawPlugin()) {") && wandMethod.includes("this.removeNoteWebWandProxy(surface);")],
  ["wand icon inherits the NoteDraw button artwork", source.includes("mwvWebWandDecorated") && source.includes(".notedraw-header-button:not(.mwv-notedraw-web-wand) > svg")],
