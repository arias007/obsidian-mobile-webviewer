...
const checks = [
  ...
  ["RealWeb fallback wand appears only in Web mode with a single wand icon", wandMethod.includes('if (!this.isNoteBrowserWebMode(surface)) {') && wandMethod.includes('setIcon(button, "wand-sparkles")') && wandMethod.includes("this.toggleNoteWebRawElementEditing(current)") && (wandMethod.match(/data-mwv-noteweb-wand/g) ?? []).length >= 3],
  ...
  ["NoteWeb reconciliation follows only an already-navigated guest", source.includes("reconcileNoteWebDocumentWithLiveSurface(embed: HTMLElement): void {") && source.includes("if (this.sameWebPage(embed.dataset.url || \"\", liveUrl)) return;") && source.includes("void this.openUrlInEmbed(embed, liveUrl, false)") && source.includes("syncRetainedLiveSurface(embed, nextUrl)")],
  ...
  ["NoteWeb Web mode always exposes a host-side magic wand", source.includes("ensureNoteWebWandProxy(surface)") && source.includes("ensureNoteWebWandProxy(embed)") && source.includes(":scope > .notedraw-webview-button:not([data-mwv-browser-more='true'])") && styles.includes(".mwv-notedraw-anchor") && styles.includes(".view-actions > .notedraw-webview-button")],
